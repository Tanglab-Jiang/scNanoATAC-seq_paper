...
# Code Layout
Generally, we ran compute-consuming programs in shell scripts, which were parallel computation at the single-cell level. After getting results from parallel computation, we made interactive data analysis mostly in the R environment.

In this project, each folder stands for a module of analysis. And in each part, if names of scripts start with step numbers (e.g. `s1` and `s2` stands for the first and second step of a pipeline), please **follow the step numbers and run them in sequential**. For those without step numbers, the scripts are parallel and independent.
